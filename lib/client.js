...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".mst_panel{--mst-blue:#4f7cff;--mst-violet:#8b5cf6;--mst-amber:#f59e0b;--mst-green:#10b981;--mst-shadow:color-mix(in srgb,var(--dsw-alias-label-primary) 18%,transparent);z-index:60;box-sizing:border-box;border:1px solid var(--dsw-alias-border-l1);background:linear-gradient(180deg,var(--dsw-alias-bg-overlay,var(--dsw-alias-bg-base)) 0%,var(--dsw-alias-bg-base) 100%);color:var(--dsw-alias-label-primary);width:480px;max-width:calc(100vw - 24px);max-height:min(760px,calc(100vh - 40px));box-shadow:0 24px 70px var(--mst-shadow),0 6px 22px color-mix(in srgb,var(--dsw-alias-label-primary) 9%,transparent);--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);border-radius:20px;flex-direction:column;display:flex;position:fixed;bottom:128px;left:12px;overflow:hidden;animation:mst_panelIn .18s cubic-bezier(.2,.8,.2,1)}",
			...
			"@media (max-width:520px){.mst_panel{left:8px;bottom:112px;width:calc(100vw - 16px);border-radius:18px}.mst_fieldGrid{grid-template-columns:1fr}.mst_footer{flex-wrap:wrap}.mst_footerHint{flex-basis:100%}.mst_button{flex:1}}"
		].join("\n");
